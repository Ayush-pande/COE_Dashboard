package com.amdocs.coe_dashboard.config;
...
public class CouchbaseConfig {
    ...
    @Value("#{systemEnvironment['DB_CONN_STR'] ?: '${spring.couchbase.bootstrap-hosts:couchbases://cb.l5qhjoisoadkul.cloud.couchbase.com}'}")
    private String host;

    @Value("#{systemEnvironment['DB_USERNAME'] ?: '${spring.couchbase.bucket.user:admin}'}")
    private String username;

    @Value("#{systemEnvironment['DB_PASSWORD'] ?: '${spring.couchbase.bucket.password:Admin@123}'}")
    private String password;

    @Value("${spring.couchbase.bucket.name:coe-dashboard}")
